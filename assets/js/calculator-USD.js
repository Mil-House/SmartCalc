...
let selectedCurrency = 'USD'; // По умолчанию выбрана AMD
...
function calculateItemTotalCost(item) {
  ...
  if (selectedCurrency === 'USD') {
    ...
    return {
      selectedPrice: unitPriceUSD, // added
    ...
    return {
      selectedPrice: unitPriceDram, // added
...
function calculateTotalCost() {
  ...
  if (selectedCurrency === 'AMD') {
    items.forEach((item) => {
      // totalCost += item.price * item.quantity;
      totalCostDram += item.price * usdRate * item.quantity;
      totalCostUSD += usdRate ? item.price * item.quantity : NaN; // Вычисляем общую стоимость в USD
    });
